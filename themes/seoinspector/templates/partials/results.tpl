...
<table id="overview">
	<tbody>
		<tr class="even" style="font-size:14px;font-weight:bold;">
			...
			<td style="color:red;text-align:center;">&bull; {if !empty($results->ugly)} {$results->ugly_count} {$heading_critical}{else}{$results->ugly_count} {$heading_critical}{/if}</td>
			<td style="color:orange;text-align:center;">&bull; {if !empty($results->bad)}{$results->bad_count} {$heading_problem}{else}{$results->bad_count} {$heading_problem}{/if}</td>
			<td style="color:green;text-align:center;">&bull; {if !empty($results->good)}{$results->good_count} {$heading_correct} {else}{$results->good_count} {$heading_correct}{/if}</td>
		</tr>
	</tbody>
</table>
<p>&nbsp;</p>
...
{if $robots_txt_enable || $sitemap_xml_enable || $canonical_tag_enable || $canonical_url_enable || $nested_tables_enable || $inline_styles_enable || $inline_script_enable || $favicon_enable || $favicon_linked_enable || $google_earth_enable || $rel_author_enable}
<h4>Miscellaneous:</h4>
