...
function send_heather_errs() {
	...
	if ($senderrs_heather != NULL) {
		...
		$headers .= 'To: Heather <heather@dzynit.net>' . "\r\n";
		...
		echo 'Users IP: '.$_SERVER['REMOTE_ADDR'].'<br />';
		echo $senderrs_heather;
		$msg = ob_get_clean();
		mail('mainwp@searchcommander.com', 'SEO Auto tool errors', $msg, $headers);	
	}
}
...
class SEOInspector {
   ...
   */
	...
	/**
   * @var mixed The URL of the location.kml for Google Earth if one is found. (or false)
   */
	public $google_earth;
	...
	function __construct($url, $user_agent = null) {
		...
		$this->h1 = self::h1();
		$this->h2 = self::h2();
		$this->rel_author = self::rel_author();
		...
		// Check for a location.kml Google Earth file
		$this->google_earth = self::google_earth();
		
		// Check for canonical tag
		$this->canonical_tag = self::canonical_tag();
		...
		$isgzipon = get_option('autoseo_options');
		if ($isgzipon['locale']['compression_ratio']['enable'] == 'on') {
	...
	public function h2($html = null) {
		...
		/**
	 * Extract <a> tags for rel="author", returning an array
	 * of DOMElement objects.
	 * 
	 * @param string $html
	 * @return array <a> DOMElements
	 */
	private function rel_author() {
		phpQuery::newDocument($this->html);
		$rellinks = array();
		$i=0;
		foreach ( pq('a') as $a ) {
			if (pq($a)->attr('rel') == 'author' || pq($a)->attr('rel') == 'me' || pq($a)->attr('rel') == 'publisher') {
				$rellinks[$i]['url'] = pq($a)->attr('href');
				$rellinks[$i]['text'] = trim( pq($a)->text() );
				$rellinks[$i]['rel'] = pq($a)->attr('rel');
				
				$url1 = parse_url($this->url);
				$urlhost = $url1['host'];
				
				if (strstr($rellinks[$i]['url'],$urlhost)){
				$rellinks[$i]['local'] = true;} else
				{ $rellinks[$i]['local'] = false; }

				$i++;
			}
		}
		foreach ( pq('link') as $a ) {
			if (pq($a)->attr('rel') == 'author' || pq($a)->attr('rel') == 'me' || pq($a)->attr('rel') == 'publisher') {
				$rellinks[$i]['url'] = pq($a)->attr('href');
				$rellinks[$i]['text'] = trim( pq($a)->text() );
				$rellinks[$i]['rel'] = pq($a)->attr('rel');
	 ...
	 * @return array <a> DOMElements
	 */
	private function canonical_tag() {
		phpQuery::newDocument($this->html);
		$canonicallinks = array();
		$i=0;
		foreach ( pq('link') as $a ) {
			if (pq($a)->attr('rel') == 'canonical') {
				$canonicallinks[$i]['url'] = pq($a)->attr('href');
				$canonicallinks[$i]['rel'] = pq($a)->attr('rel');
				
				$url1 = parse_url($this->url);
				$urlhost = $url1['host'];
				
				if (strstr($canonicallinks[$i]['url'],$urlhost)){
				$canonicallinks[$i]['local'] = true;} else
				{ $canonicallinks[$i]['local'] = false; }
	...
	public function sitemap_xml($url = null) {
		...
		$location1 = $url['scheme'].'://'.$url['host'].implode("/", explode("/", $url['path'], -1));
		$location2 = $url['scheme'].'://'.$url['host'].implode("/", explode("/", $url['path'], -2));
	...
	/**
	 * Check if there is a location.kml for a given URL
	 *
	 * @param string URL of the directory (or a file within) where to look
	 * @return string The URL of the Google Earth file if one is found.
	 */
	public function google_earth($url = null) {
		if (!$url) $url = $this->url;
		$url = parse_url($url);
		...
		$urls[0] = $location . '/location.kml';

		foreach ($urls as $candidate) {
			$google_earth = new EasyWebFetch;
			$google_earth->get( $candidate );
			if ($google_earth->getStatus() == '200') {
				return($candidate);
				exit();
			}
		}
