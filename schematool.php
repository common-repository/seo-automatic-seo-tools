...
if ( isset( $_POST['submit'] ) ) { 
	...
	if ($_REQUEST['logourl'] == '' || $_REQUEST['url'] == '' || $_REQUEST['imageurl'] == '') { 
		if ($_REQUEST['url'] == '') {
		...
		if ($_REQUEST['imageurl'] == '') {
			$reqlogourl = '<b><font color="#ff0000">Image URL is required</font></b><br /><br />';
		}
	} else {
		...
		if ($_REQUEST['imageurl']) { $imageurl = '"image" : "'.$_REQUEST['imageurl'].'",'; }
		...
		if ($staddress != '' || $city != '' || $state != '' || $zip != '') {
			$address = '"address": {"@type": "PostalAddress",'.$staddress.$city.$state.$zip.'},';
		}
		...
		$thecode = '<script type="application/ld+json">{"@context" : "http://schema.org",'.$bustype.$busname.$phone.$address.$descript.$url.$logourl.$imageurl.$openinghours.$schemasocials.'}</script>';
...
.schematooltip .schematooltiptext {
    visibility: hidden;
    width: 175px;
...
.schematooltip:hover .schematooltiptext {
    visibility: visible;
}
</style>
...
<span><label class="lbl"><div class="schematooltip">Image URL <font color="#ff0000">* Required &#9432;</font> <span class="schematooltiptext">This MAY be the same url of your logo, or anything else you want.</span></div></label></span><input id="imageurl" name="imageurl" class="inputfld text" value="'.$_REQUEST['imageurl'].'" /><br /><br />
...
<input id="getcode" class="button_text" type="submit" name="submit" value="Submit" /><br /><br />
	...
	if(get_option('seo_tools_linkback_seotools') == 'on') {
		$seotools = '<p>&nbsp;</p><small><a href="http://www.seoautomatic.com/unique-tools/structured-data-builder/" target="_blank">This Structured Data Tool for Local Businesses was provided by Search Commander Inc.</a></small>';
	} else {
		$seotools = '';
	}
